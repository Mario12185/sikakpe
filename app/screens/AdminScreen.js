        ...
        <View style={{padding:12,marginBottom:8,borderRadius:8,backgroundColor:'#fff'}}>
          ...
          {item.status==='pending' && (
            <View style={{flexDirection:'row',marginTop:8}}>
              <TouchableOpacity onPress={()=>update(item.id,'verified')} style={{flex:1,backgroundColor:'#00aa55',padding:8,marginRight:4,borderRadius:6}}><Text style={{color:'#fff'}}>✅</Text></TouchableOpacity>
              <TouchableOpacity onPress={()=>update(item.id,'rejected')} style={{flex:1,backgroundColor:'#c53030',padding:8,borderRadius:6}}><Text style={{color:'#fff'}}>❌</Text></TouchableOpacity>
            </View>
          )}
        </View>
      )} />
